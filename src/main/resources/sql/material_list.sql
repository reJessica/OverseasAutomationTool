...
CREATE TABLE material_list (
    ...
    `CER_History_FY_TYPE_Class_CER` TEXT COMMENT '证书历史(FY-TYPE-Class-CER)',
    ...
    `SFDALicense_备注_注册证备注信息` TEXT COMMENT 'SFDA许可证:备注(注册证备注信息)',
    ...
    `Change_EN_VS_CN` TEXT COMMENT '英文与中文变更',
    `Released_IFU_no` VARCHAR(255) COMMENT '发布IFU编号',
    `Released_Label_No` VARCHAR(255) COMMENT '发布标签编号',
    `Released_Label_Date` VARCHAR(255) COMMENT '发布标签日期',
    `UDI_Database_Description` TEXT COMMENT 'UDI数据库描述',
