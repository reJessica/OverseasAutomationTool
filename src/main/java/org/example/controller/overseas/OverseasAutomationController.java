...
@RestController
@CrossOrigin
@RequestMapping("/overseas/automation")
public class OverseasAutomationController {
    private static final Logger logger = LoggerFactory.getLogger(OverseasAutomationController.class);
    ...
    public ResponseEntity<String> stopService() {
        try {
            ...
            logger.info("服务已成功停止");
            return ResponseEntity.ok("服务已停止，若要继续，请重新启动软件（请勿再次点击启动服务按钮）");
        } catch (Exception e) {
