...
public interface OverseasReportMapper {
    ...
    @Insert("INSERT INTO overseas_reports (report_number, report_path, status, report_no, report_no_en, report_date, report_date_en, " +
            "reporter, reporter_en, customer_name, customer_name_en, address, address_en, " +
            "contact_person, contact_person_en, tel, tel_en, occurrence_place, occurrence_place_en, " +
            ...
            "drug_device_comb_desc, drug_device_comb_desc_en, investigation_flag, investigation_flag_en, " +
            "investigation_desc, investigation_desc_en, relative_evaluation, relative_evaluation_en, " +
            "event_reason_analysis, event_reason_analysis_en, need_risk_assessment, need_risk_assessment_en, " +
            "plan_submit_date, plan_submit_date_en, has_control_measure, has_control_measure_en, " +
            "control_measure_details, control_measure_details_en, no_control_measure_reason, no_control_measure_reason_en) " +
            "VALUES (#{reportNumber}, #{reportPath}, #{status}, #{reportNo}, #{reportNoEn}, #{reportDate}, #{reportDateEn}, " +
            "#{reporter}, #{reporterEn}, #{customerName}, #{customerNameEn}, #{address}, #{addressEn}, " +
            "#{contactPerson}, #{contactPersonEn}, #{tel}, #{telEn}, #{occurrencePlace}, #{occurrencePlaceEn}, " +
            ...
            "#{drugDeviceCombDesc}, #{drugDeviceCombDescEn}, #{investigationFlag}, #{investigationFlagEn}, " +
            "#{investigationDesc}, #{investigationDescEn}, #{relativeEvaluation}, #{relativeEvaluationEn}, " +
            "#{eventReasonAnalysis}, #{eventReasonAnalysisEn}, #{needRiskAssessment}, #{needRiskAssessmentEn}, " +
            "#{planSubmitDate}, #{planSubmitDateEn}, #{hasControlMeasure}, #{hasControlMeasureEn}, " +
            "#{controlMeasureDetails}, #{controlMeasureDetailsEn}, #{noControlMeasureReason}, #{noControlMeasureReasonEn})")
    ...
    @Update("UPDATE overseas_reports SET report_date = #{reportDate}, status = #{status} WHERE id = #{id}")
    void update(OverseasReport report);

    @Update("UPDATE overseas_reports SET status = #{status} WHERE id = #{id}")
